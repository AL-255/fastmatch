...
def params_to_dict(p: MatchParams) -> dict:
    ...
    return {
        ...
        "rgb_weights": [float(w) for w in p.rgb_weights],
        "ycbcr_weights": [float(w) for w in p.ycbcr_weights],
        "method": str(p.method),
...
def params_from_dict(d: dict) -> MatchParams:
    ...
    return MatchParams(
        ...
        rgb_weights=tuple(float(w) for w in d.get("rgb_weights", defaults.rgb_weights)),
        ycbcr_weights=tuple(float(w) for w in d.get("ycbcr_weights", defaults.ycbcr_weights)),
